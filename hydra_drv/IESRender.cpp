...
std::vector<float> CreateTextureIESTypeC(oldies::IE_DATA, int* pW, int* pH);
std::vector<float> CreateTextureIESTypeB(oldies::IE_DATA, int* pW, int* pH);
...
std::vector<float> CreateSphericalTextureFromIES(const std::string& a_iesData, int* pW, int* pH)
{
	...
	if (iesOldCrap.photo.gonio_type == oldies::Type_B)
	{
		return CreateTextureIESTypeB(iesOldCrap, pW, pH);
	}
	else if (iesOldCrap.photo.gonio_type == oldies::Type_C)
	{
		return CreateTextureIESTypeC(iesOldCrap, pW, pH);
	}
	else if (iesOldCrap.photo.gonio_type == oldies::Type_A)
	{
		std::cerr << "oldies::IE_ReadFile error" << std::endl;
		(*pW) = 1;
		(*pH) = 1;
...
std::vector<float> CreateTextureIESTypeB(oldies::IE_DATA iesOldCrap, int* pW, int* pH)
{
	int w = 0; // phi, 0-360
	int h = 0; // theta, 0-180

	float verticalStart = iesOldCrap.photo.vert_angles[0];
	float verticalEnd = iesOldCrap.photo.vert_angles[iesOldCrap.photo.num_vert_angles - 1]; // 0-90, 90-180, 0-180, -90-90

	float horizontStart = iesOldCrap.photo.horz_angles[0];
	float horizontEnd = iesOldCrap.photo.horz_angles[iesOldCrap.photo.num_horz_angles - 1]; // 0-0, 0-90, 0-180, 180-360, 0-360, -90-90
	...
	if (fabs(verticalStart) < eps && fabs(verticalEnd - 90.0f) < eps) // 0-90
		...
		h = iesOldCrap.photo.num_vert_angles * 2;
	else if (fabs(verticalStart + 90.0f) < eps && fabs(verticalEnd - 90.0f) < eps) // -90-90
	...
	else if (fabs(horizontStart + 90) < eps && fabs(horizontEnd - 90.0f) < eps) // -90-90, If it is 90�, the distribution is symmetric about a horizontal plane.
	{
		w = iesOldCrap.photo.num_horz_angles * 4;
		reflectType = REFLECT4;
	}
	...
	/*for (size_t i = 0; i < iesOldCrap.photo.num_vert_angles; i++)
	{
		for (size_t j = 0; j < iesOldCrap.photo.num_horz_angles; j++)
		{
			resultData[i*w + j] = iesOldCrap.photo.pcandela[j][i];
		}
	}*/
	...
	if (reflectType == REFLECT4)
	{
		for (float thetaGrad = verticalStart; (thetaGrad <= verticalEnd); thetaGrad += stepTheta)
		{
			...
			for (float phiGrad = -90.0f; (phiGrad <= 90.0f); phiGrad += stepPhi)
			{
				if (phiGrad < 0)
					phiGrad += 360;
	...
	else if (reflectType == REFLECT2)
	{
		for (float thetaGrad = verticalStart; (thetaGrad <= verticalEnd); thetaGrad += stepTheta)
		{
			...
			for (float phiGrad = -90.0f; (phiGrad <= 180.0f); phiGrad += stepPhi)
			{
				if (phiGrad < 0)
					phiGrad += 360;
...
std::vector<float> CreateTextureIESTypeC(oldies::IE_DATA iesOldCrap, int* pW, int* pH)
{
	int w = 0; // phi, 0-360
	int h = 0; // theta, 0-180
